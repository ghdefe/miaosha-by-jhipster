...
class MiaoshaByJhipsterKafkaResourceIT {
    ...
    void setup() {
        ...
        MiaoshaByJhipsterKafkaResource kafkaResource = new MiaoshaByJhipsterKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/miaosha-by-jhipster-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/miaosha-by-jhipster-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
